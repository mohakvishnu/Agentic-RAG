...
function ChatPage() {
  ...
  const [log, setLog] = useState([]) // [{role:'user',text}, {role:'assistant',answer,citations,route}]
  ...
  return (
    <div className="content">
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      code({node, inline, className, children, ...props}) {
                        ...
                        return (
                          <code
                            style={{ background:"#e2e8f0", padding:"2px 6px", borderRadius:"6px", fontSize:"0.92em", color:"#111827" }}
                            {...props}
                          >
                            {children}
                          </code>
                      ...
                      <ul style={{margin:0,paddingLeft:16}}>
                        {m.citations.map((c,idx)=>(
                          <li key={idx} style={{color:"#64748b"}}>
                            <code>{c.doc_id}</code>{c.meta?.source ? ` — ${c.meta.source}` : ""}
                          </li>
                        ))}
                      </ul>
                    </details>
      ...
      <form onSubmit={onAsk} className="inputbar">
        ...
        />
        <button className="btn primary" disabled={busy}>{busy ? "Thinking…" : "Send"}</button>
      </form>
    </div>
  )
}
...
function UploadPage(){
  ...
  async function onUpload(){
    ...
    try{ const res = await uploadFiles(files); setLast(res); alert(`Uploaded: ${res.uploaded}, Ingested: ${res.ingested}`); ref.current.value="" }
    catch(e){ alert(e.message) }
  }
  return (
    <div className="content">
      ...
      <div className="panel" style={{margin:16}}>
        ...
        <div style={{marginTop:10,display:"flex",gap:8}}>
          ...
          <button className="btn" onClick={async ()=>{
            const text = prompt("Quick ingest text:"); if(text){ await ingest(text); alert("Ingested!") }
          }}>Quick Ingest</button>
        </div>
...
function DocsPage(){
  ...
  useEffect(()=>{ (async()=> setData(await listDocs(page,size)))() }, [page,size])
    ...
    <div className="content">
      ...
      <div style={{display:"grid",gap:16,gridTemplateColumns:"1fr 1fr",margin:16}}>
        ...
        <div className="panel">
          ...
          <div className="muted">Hybrid search (Chroma dense + BM25 sparse) with RRF. Uploads feed both stores.</div>
        </div>
      </div>
    </div>
  )
